...
bool JlCompress::compressFile(QuaZip* zip, QString fileName, QString fileDest, QString password ) {
    ...
    // Apro il file risulato
    QuaZipFile outFile(zip);
    if(!outFile.open(QIODevice::WriteOnly, QuaZipNewInfo(fileDest, inFile.fileName()),password.toStdString().c_str(),0,0,Z_NO_COMPRESSION)) return false;
...
bool JlCompress::compressSubDir(QuaZip* zip, QString dir, QString origDir, QDir::Filters filters, bool recursive) {
    // zip: oggetto dove aggiungere il file
    ...
    if (recursive) {
        ...
        Q_FOREACH (QFileInfo file, files) {
            ...
            if(!compressSubDir(zip,file.absoluteFilePath(),origDir,filters,recursive)) return false;
        }
    }
...
bool JlCompress::compressSubDir(QuaZip* zip, QString dir, QString origDir, QString password, bool recursive,
                                QStringList m_vAttachFiles,
                                QStringList m_vFileLibraryFiles,
                                QStringList m_vFileIndexFiles ) {
    // zip: oggetto dove aggiungere il file
    // dir: cartella reale corrente
    // origDir: cartella reale originale
    // (path(dir)-path(origDir)) = path interno all'oggetto zip
    ...
    if (recursive) {
        ...
        Q_FOREACH (QFileInfo file, files) {
            // Comprimo la sotto cartella
            if(!compressSubDir(zip,file.absoluteFilePath(),origDir,password,recursive,m_vAttachFiles,m_vFileLibraryFiles,m_vFileIndexFiles)) return false;
        }
    }

    // Per ogni file nella cartella
    QFileInfoList files = directory.entryInfoList(QDir::Files);
    if (directory.absolutePath().indexOf("attachments") >= 0 || directory.absolutePath().indexOf("filelibrary") >= 0 || directory.absolutePath().indexOf("index") >= 0)
        return true;
    Q_FOREACH (QFileInfo file, files) {
        // Se non e un file o e il file compresso che sto creando
        if(!file.isFile()||file.absoluteFilePath()==zip->getZipName()) continue;
        ...
        // Comprimo il file
        if (!compressFile(zip,file.absoluteFilePath(),filename,password.toStdString().c_str())) return false;
    }
...
bool JlCompress::compressDir(QString fileCompressed, QString dir, bool recursive, QString password,
                             QStringList m_vAttachFiles,
                             QStringList m_vFileLibraryFiles,
                             QStringList m_vFileIndexFiles ){
    // Creo lo zip
    ...
    if (!compressSubDir(&zip,dir,dir,password, recursive,m_vAttachFiles,m_vFileLibraryFiles,m_vFileIndexFiles)) {
        QFile::remove(fileCompressed);
        return false;
    }
    int i = 0;
    for( i = 0; i< m_vAttachFiles.length(); i++ ) {
        QFileInfo file( m_vAttachFiles.at(i) );
        QString attachments = "attachments/";
        attachments += file.fileName();
        if ( !compressFile(&zip, file.absoluteFilePath(), attachments, password ) ) {
        }
    }
    for( i = 0; i< m_vFileLibraryFiles.length(); i++ ) {
        QFileInfo file( m_vFileLibraryFiles.at(i) );
        QString filelibrary = "filelibrary/";
        filelibrary += file.fileName();
        // Comprimo il file
        if ( !compressFile(&zip, file.absoluteFilePath(), filelibrary, password ) ) {
        }
    }

    for( i = 0; i< m_vFileIndexFiles.length(); i++ ) {
        QFileInfo file( m_vFileIndexFiles.at(i) );
        QString filelibrary = "index/";
        filelibrary += file.fileName();
        // Comprimo il file
        if ( !compressFile(&zip, file.absoluteFilePath(), filelibrary, password ) ) {
        }
                             ...
                             bool recursive, QDir::Filters filters)
{
    ...
    // Aggiungo i file e le sotto cartelle
    if (!compressSubDir(&zip,dir,dir, filters,recursive)) {
        QFile::remove(fileCompressed);
        return false;
    }
